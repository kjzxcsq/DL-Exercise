...
class FullyConnected(BaseLayer):
    ...
    def initialize(self, weights_initializer, bias_initializer):
        """
        Reinitializes the weights and biases of the layer using the given initializers.

        Args:
            weights_initializer: The initializer object for weights
            bias_initializer: The initializer object for biases
        """
        # Initialize weights and biases separately, then combine
        weight_shape = (self.input_size, self.output_size)
        bias_shape = (1, self.output_size)
        weights = weights_initializer.initialize(weight_shape, fan_in=weight_shape[0], fan_out=weight_shape[1])
        biases = bias_initializer.initialize(bias_shape, fan_in=None, fan_out=None)
